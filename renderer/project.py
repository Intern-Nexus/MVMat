...
class UVProjection():
	...
	def load_mesh(self, mesh_path, scale_factor=2.0, auto_center=True, autouv=False):
		...
		if autouv:
			...
			_, tmp_faces, tmp_aux = load_obj(mesh_path)
			verts_uvs = tmp_aux.verts_uvs.to(mesh.device)  # (V, 2)
			faces_uvs = tmp_faces.textures_idx.to(mesh.device)  # (F, 3)
	...
	def calculate_visible_triangle_mask(self, channels=None, image_size=(512,512)):
		...
		def get_all_visible_triangles(all_pix2face):
			all_pix2face = all_pix2face.clone()
			# +1, put background -1 to 0, and put visible triangles to 1 ~ N+1, clamp it to 0-1 mask
			all_visible_triangles = (all_pix2face + 1).clamp(min=0, max=1)
			all_visible_triangles[:,1:][all_visible_triangles[:,:-1] > 0] = 1
			all_visible_triangles[:,:-1][all_visible_triangles[:,1:] > 0] = 1
			all_visible_triangles[1:,:][all_visible_triangles[:-1,:] > 0] = 1
			all_visible_triangles[:-1,:][all_visible_triangles[1:,:] > 0] = 1

			return all_visible_triangles

		def get_visible_triangles(valid_faceid):
		...
		visible_triangles = []
		cos_threshold = 0.5
		cos_sum_threshold = 0.9
		N_face = len(self.mesh.faces_list()[0])
		face_cos_sum = torch.zeros([N_face]).float().cuda()

		_,_,_, cos_maps, _,_ = self.render_geometry(image_size=image_size)
		cos_maps = cos_maps[..., 0:1] # shape: [N, H, W, 1]
		valid_triangles = []
		for i in range(len(pix2face_list)):
			cur_faceid = pix2face_list[i][0] # shape: [H, W, 1]
			cur_cos_map = cos_maps[i]        # shape: [H, W, 1]
			# ensure the visibility of the faces with high confidence (cos values)
			cur_cos_map[cur_cos_map >= cos_threshold] = cos_sum_threshold + 0.1
			cur_face_cos_avg = scatter_mean(cur_cos_map[cur_faceid > -1], cur_faceid[cur_faceid > -1], dim=-1)
			assert len(cur_face_cos_avg) <= N_face
			face_cos_sum[:len(cur_face_cos_avg)] += cur_face_cos_avg

			valid_faceid = torch.unique(cur_faceid)
		...
		valid_triangles = torch.unique(torch.cat(valid_triangles))
		self.visible_triangles = visible_triangles
		self.all_visible_triangles = get_all_visible_triangles(uv_pix2face[0])
